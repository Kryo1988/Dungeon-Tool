-- luacheck: globals KryosDungeonTool GAMEMENU_OPTIONS MenuResponse MenuUtil ClassTalentHelper PlayerSpellsMicroButton
...
local function GetCurrentTalents(stream)
	...
	if not db.hideIcon then
		...
		icon = ("|TInterface\\Addons\\KryosDungeonTool\\Icons\\Talents:%d:%d:0:0|t "):format(size, size)
	end
