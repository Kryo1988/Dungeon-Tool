-- luacheck: globals KryosDungeonTool GAMEMENU_OPTIONS STAT_HASTE STAT_MASTERY STAT_VERSATILITY STAT_CRITICAL_STRIKE CR_HASTE_MELEE CR_MASTERY CR_VERSATILITY_DAMAGE_DONE CR_VERSATILITY_DAMAGE_TAKEN CR_CRIT_MELEE CR_LIFESTEAL CR_BLOCK CR_PARRY CR_DODGE CR_AVOIDANCE CR_SPEED STAT_LIFESTEAL STAT_BLOCK STAT_PARRY STAT_DODGE STAT_AVOIDANCE STAT_SPEED GetLifesteal GetBlockChance GetParryChance GetDodgeChance GetAvoidance GetSpeed
...
local function addStatOptions(frame, key, label, supportsMode)
	local group = GetAceGUI():Create("InlineGroup")
	group:SetTitle(label)
	group:SetFullWidth(true)
	group:SetLayout("List")

	local show = GetAceGUI():Create("CheckBox")
	show:SetLabel(SHOW)
	...
	if supportsMode ~= false then
		local mode = GetAceGUI():Create("Dropdown")
	...
	local color = GetAceGUI():Create("ColorPicker")
...
local function createAceWindow()
	...
	local vertical = GetAceGUI():Create("CheckBox")
...
stream = KryosDungeonTool.DataHub.RegisterStream(provider)
